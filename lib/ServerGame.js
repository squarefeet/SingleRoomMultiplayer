...
ServerGame.prototype = {
	...
	onStateReceived: function( state ) {
		...
		if(state.moveState.isFiring) {
			this.primaryWeapon.burstFire(
				this.players[ state.name ].player,
				state.name,
				this.players[ state.name ].player.middlegroundTarget.position,
				this.players[ state.name ].player.middlegroundTarget.quaternion
			);
		}
		else {
	...
	tick: function() {
		...
		for( var i in this.players ) {
			this.players[i].player.updateMatrix( delta );
		}
