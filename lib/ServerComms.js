...
var sio = require('socket.io'),
	ServerGame = require('./ServerGame.js'),
	noop = function() {},
	playerNames = {};
...
ServerComms.prototype = {
	...
	onConnect: function( socket ) {
		...
		// Create a game and start it if there's not one already running.
		if(!this.game) {
	...
	onPacketReceived: function( packet ) {
		...
		this.game.onStateReceived( state );
	...
	onDisconnect: function() {
		...
		if( playerNames[this.playerName.name] ) {
			console.log('removing', this.playerName.name);
