...
audio.io.Waveshaper = audio.io.Effect.extend({
	...
	initialize: function() {
		...
		that.curve = null;
		that.shaper = that._io.context.createWaveShaper();

		that.createCurve();

		that.input.connect( that.shaper );
		that.shaper.connect( that.wet );



		that.on('change', function(model, value) {
			that.createCurve();
		});
	},
	...
	createCurve: function() {
		var samples = this.get('samples'),
			...
			curve = new Float32Array(this.get('samples'));
